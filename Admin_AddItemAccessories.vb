...
Public Class Admin_AddItemAccessories
    ...
    Private Sub btn_AddingItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_AddingItem.Click
        ...
        Else
            'Using pkID As New OleDbCommand("Select Count([ID]) from tbl_Accessories", connection)
            '    Dim countA As Integer
            '    countA = pkID.ExecuteScalar().ToString
            '    txt_ID.Text = countA + 1
            'End Using
            'Using cmd As New OleDbCommand("Select Count(*) from tbl_Accessories where [ID] = @ID or [Equipments] = @Equipments", connection)
            ...
            Using create As New OleDbCommand("insert into tbl_Accessories([ID],[Equipments],[Condition],[Cost],[StatusDelete], [Availability],[EquipmentDetails]) Values(@ID,@Equipments,@Condition,@Cost, @StatusDelete, @Availability,@EquipmentDetails)", connection)
